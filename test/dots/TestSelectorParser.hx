...
import dots.SelectorParser.SelectorParserResult;
import utest.Assert;
using thx.Iterators;
using thx.Maps;
...
class TestSelectorParser {
  ...
  public function testParseSelectorAttributeBool() {
    ...
    assertTag("div", result);
    assertAttrs(["disabled" => "disabled"], result);
  }
  ...
  inline function assertAttrs(expected : Map<String, String>, actual : SelectorParserResult) : Void {
